...
function ChannelEditScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const fetchChannel = async () => {
      try {
        ...
        setChannelOwnerId(channel.ownerId);
        if (channel.avatar) {
          setCarouselImages([`${BASE_URL}${channel.avatar}`]);
        }
      } catch (err) {
        console.log("Ошибка при загрузке канала:", err.response?.data || err.message);
      }
    };
    fetchChannel();
  }, [channelId]);
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        ...
        console.log("Ошибка при загрузке userId:", err);
      }
    };
    fetchCurrentUser();
  }, []);
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      if (currentUserId && channelOwnerId && Number(currentUserId) === Number(channelOwnerId)) {
        try {
          const resMembers = await axios.get(`${BASE_URL}/api/channels/members/${channelId}`);
          setMembers(resMembers.data);
        } catch (err) {
          console.log("Ошибка при загрузке участников канала:", err.response?.data || err.message);
        }
      }
    };
    fetchMembers();
  }, [currentUserId, channelOwnerId, channelId]);
  ...
  const pickImage = async () => {
    if (Platform.OS !== "web") {
      ...
      if (!result.cancelled) {
        setCarouselImages([result.uri, ...carouselImages]);
      }
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axios.put(`${BASE_URL}/api/channels/${channelId}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      socket.emit("channelUpdated", {
        channelId,
        updatedFields: { name: channelName, avatar: res.data.avatar },
      });

      Alert.alert("Канал обновлён", "Изменения сохранены", [
        {
          text: "OK",
          onPress: () => navigation.navigate("ChannelProfileScreen", { channelId }),
        },
      ]);
    } catch (error) {
      console.log("Ошибка при обновлении канала:", error.response?.data || error.message);
      Alert.alert("Ошибка", "Не удалось обновить канал");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getAvatarColor = (username) => {
    const colors = ['#7C5CFF', '#00B4DB', '#FF6B6B', '#00D2D3', '#AD94FF', '#FD79A8', '#FDCB6E'];
  ...
  return (
    <View style={[styles.screen, { backgroundColor: themeColors.background }]}>
      ...
      >
        ...
        <View style={[styles.formCard, { backgroundColor: themeColors.card }]}>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={[styles.inputWrapper, { backgroundColor: themeColors.inputBg, borderColor: themeColors.inputBorder }]}>
              <Ionicons name="megaphone-outline" size={18} color={themeColors.subtext} style={styles.inputIcon} />
              <TextInput
            ...
            <View style={styles.switchInfo}>
              ...
              <View>
                ...
                <Text style={[styles.switchSubtext, { color: themeColors.subtext }]}>
                  {isPublic ? "Виден всем пользователям" : "Только по приглашению"}
                </Text>
              </View>
            </View>
            <Switch
        ...
        {/* Members Section */}
        {currentUserId === String(channelOwnerId) && members.length > 0 && (
          <View style={[styles.membersCard, { backgroundColor: themeColors.card }]}>
            ...
            {members.map((member) => (
              <View key={member.id} style={[styles.memberItem, { backgroundColor: themeColors.cardElevated }]}>
