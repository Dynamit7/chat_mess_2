...
export default function GroupInfoScreen() {
  ...
  const name = group?.name || params.name || t('info.group');
  ...
  return (
    <AuroraBackground palette={c}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={styles.headerTitle}>{t('info.group')}</Text>
        {isOwner ? (
          <Pressable
            hitSlop={12}
      ...
      {loading ? (
        ...
        >
          ...
          <View style={styles.hero}>
            ...
            <Text style={styles.heroSub}>{t('info.members', { count: memberCount })} · {t('info.group')}</Text>
          </View>

          {/* Quick actions */}
          <View style={styles.actions}>
            <ActionChip icon="chatbubbles" label={t('info.open')} onPress={openChat} styles={styles} c={c} />
            {isOwner && (
              <ActionChip icon="person-add" label={t('info.add')} onPress={() => setAdding(true)} styles={styles} c={c} />
            )}
            {isMember && (
              <ActionChip icon="exit-outline" label={t('info.leave')} onPress={leave} danger loading={leaving} styles={styles} c={c} />
            )}
          </View>
          ...
          <GlassCard padded={false} palette={c}>
            <InfoRow icon="people-outline" label={t('info.membersLabel')} value={String(memberCount)} styles={styles} c={c} />
            <InfoRow icon="lock-closed-outline" label={t('info.type')} value={group?.isPublic ? t('info.publicF') : t('info.privateF')} last styles={styles} c={c} />
          </GlassCard>
              ...
              <GlassCard padded={false} palette={c}>
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        {(m.role === 'owner' || m.isOwner || Number(m.userId ?? m.id) === group?.ownerId) && (
                          <Text style={styles.memberRole}>{t('info.owner')}</Text>
                        )}
                      </View>
                ...
                {members.length > 10 && (
                  <View style={styles.moreRow}>
                    <Text style={styles.moreText}>{t('info.moreMembers', { count: members.length - 10 })}</Text>
                  </View>
                )}
              </GlassCard>
            </>
