      ...
      {loading ? (
        ...
        >
          ...
          <View style={styles.actions}>
            <ActionChip icon="chatbubbles" label="Открыть" onPress={openChat} />
            {isMember && (
              <ActionChip icon="exit-outline" label="Покинуть" onPress={leave} danger loading={leaving} />
            )}
          </View>
          ...
          <GlassCard padded={false}>
            <InfoRow icon="people-outline" label="Участников" value={String(memberCount)} />
            <InfoRow icon="lock-closed-outline" label="Тип" value={group?.isPublic ? 'Публичная' : 'Приватная'} last />
          </GlassCard>
...
const styles = StyleSheet.create({
  ...
  moreText: { color: colors.textFaint, fontFamily: font.bodyMed, fontSize: 14 },
});
