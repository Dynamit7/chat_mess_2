...
export function VoiceMessage({ uri, isOut }: { uri: string; isOut: boolean }) {
  ...
  const fg = isOut ? c.ink : c.accent;
  const track = isOut ? `${c.ink}44` : c.stroke2;

  return (
    <Pressable onPress={toggle} style={styles.row}>
      <View style={[styles.btn, { backgroundColor: isOut ? `${c.ink}22` : c.accentSoft }]}>
      ...
      <View style={styles.body}>
        ...
        <Text style={[styles.time, { color: isOut ? `${c.ink}99` : c.textFaint }]}>
