...
export default function EditEntity() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      Alert.alert(t('edit.errorTitle'), e?.response?.data?.error || t('edit.saveChangesFailed'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <AuroraBackground palette={c}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        <Text style={styles.title}>{t('msg.edit')}</Text>
      ...
      {fetching ? (
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
          ...
          >
            ...
            <TextField
              label={t('edit.entityName')}
              icon={isGroup ? 'people-outline' : 'megaphone-outline'}
              placeholder={isGroup ? t('edit.groupNamePh') : t('edit.channelNamePh')}
              value={name}
            ...
            <View style={styles.toggleRow}>
              <View style={{ flex: 1 }}>
                <Text style={styles.toggleLabel}>{t('info.public')}</Text>
                <Text style={styles.toggleSub}>
                  {isPublic ? t('edit.publicOn') : t('edit.publicOff')}
                </Text>
              </View>
              <Switch
                value={isPublic}
